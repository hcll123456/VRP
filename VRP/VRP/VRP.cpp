// VRP.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
#include <fstream>
#include <sstream>
#include <algorithm>
...
double vehicleInfoMatrix[K + 2][3];// K下标从1开始到K，0列表示车的最大载重量，1列表示车行驶的最大距离，2列表示速度
int* decodedArr;// 染色体解码后表达的每辆车的服务的客户的顺序
double distanceMatrix[clientNum + 1][clientNum + 1];// 客户距离
double* weightArr;// 客户需求量
int oldMatrix[populationScale][clientNum+K];// 初始种群，父代种群，行数表示种群规模，一行代表一个个体，即染色体，列表示染色体基因片段
int newMatrix[populationScale][clientNum+K];// 新的种群，子代种群
...
private:
    double bestFitness;
    int bestGenerationNum;
public:
    int getBestGenerationNum() {
        return bestGenerationNum;
    }
    void setBestGenerationNum(int bestGenerationNum) {
...
void decoding(int* Gh) {
    ...
    for (j = 1; j < clientNum; j++) {
        ...
        if (cur_q > vehicleInfoMatrix[i][0]
            ...
            evaluation =
                evaluation + cur_d - distanceMatrix[Gh[j]][Gh[j - 1]] + distanceMatrix[Gh[j - 1]][0];
            cur_d = distanceMatrix[0][Gh[j]];// 从配送中心到当前客户j距离
            cur_q = weightArr[Gh[j]];
        }
        else {
...
void initGroup() {
    ...
    for (k = 0; k < populationScale; k++)// 种群数
    {
        for (i = 0; i < clientNum; i++)
            oldMatrix[k][i] = i + 1;
        for (i = 0; i < clientNum; i++) {
            randomNum = (rand() % clientNum);
            swap(oldMatrix[k][i], oldMatrix[k][randomNum]);
    ...
    //c++
    // 显示初始化种群
    //cout << "///////////////显示初始种群开始(In initGroup method)////////////////////"<<endl;
...
void copyChrosome(int k, int kk) {
    ...
    for (int i = 0; i < clientNum + K; i++)
        newMatrix[k][i + 0] = oldMatrix[kk][i + 0];
...
void selectBestChrosome() {
    ...
    if (bestFitness < maxevaluation) {
        ...
        for (int i = 0; i < clientNum + K; i++)
            bestGhArr[i + 0] = oldMatrix[maxid][i + 0];
...
int select() {
    ...
    int k;
    double ran1;
    ran1 = (rand() % 1000) * 0.001;
    //ran1 = abs(ra.nextDouble());
...
BestResult* solveVrp() {
    ...
    cout<<"\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\t"<<endl;
    // 出现代数
    cout<<"最好的代数出现在："<< bestGenerationNum <<"代"<<endl;
    // 染色体评价值
    cout<<"最好的结果为：" <<(10 / bestFitness) << "或" <<bestFitness;
    // 最好的染色体
    for (int i = 0; i < clientNum; i++)
        cout << "[" << bestGhArr[i] << "]  ";
    cout << endl;
    ...
    for (int i = 1; i <= KK; i++)
        cout << "[" << decodedArr[i] << "]  ";
    cout << endl;
    cout<<"车辆行驶距离解码：" << decodedEvaluation<<endl;
...
*/
    ...
    for (i = 1; i <= KK; i++) {
        ...
        for (j = tek, k = 2; j < decodedArr[i]; j++, k++) {
            tefa = tefa + to_string(bestGhArr[j]) + "-";
            templ[k] = bestGhArr[j];
        }
        templ[k] = 0;
        templ[0] = k;
...
int main()
{
    ...
    for (int i = 0; i < count; i++) {
        cout << "/////////the " << (i + 1) <<"iteration start...////////" << endl;
/*        System.out.println(
...
*/
        ...
        cout << "/////////the " << (i + 1) << "iteration end...////////" << endl << endl;
/*
        System.out.println(
...
*/
    ...
    cout << "平均在第" << (generationNum / count) << "代找到最有解。" << endl;
    cout << "平均的路成为：" << (totalFitness / count) << endl;
/*
